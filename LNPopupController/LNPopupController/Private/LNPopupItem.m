...
- (instancetype)init
{
	...
	if(self)
	{
        ...
        [self addObserver:self forKeyPath:NSStringFromSelector(@selector(imgThumb)) options:0 context:_LNPopupItemObservationContext];
...
- (void)dealloc
{
    ...
    [self removeObserver:self forKeyPath:NSStringFromSelector(@selector(imgThumb)) context:_LNPopupItemObservationContext];
